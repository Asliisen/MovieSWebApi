...
namespace WebApi.Applications.ActorOperations.Commands.UpdateActor
{
    public class UpdateActorCommand
    {
        ...
        public void Handle()
        {
            ...
            // Check if there is already an actor with the given name and last name.
            if (_context.Actors.Any(x => x.Name.ToLower() == Model.Name.ToLower() && x.LastName.ToLower() == Model.LastName.ToLower() && x.Id != ActorId))
                throw new InvalidOperationException("There is already an actor with the given name.");

            // Update the actor's name and last name if provided in the model.
