...
namespace WebApi.Applications.GenreOperations.Commands.UpdateGenre
{
    public class UpdateGenreCommand
    {
        ...
        public void Handle()
        {
            ...
            // Check if there is already a genre with the given name (case-insensitive).
            if (_context.Genres.Any(x => x.Name.ToLower() == Model.Name.ToLower() && x.Id != GenreId))
                throw new InvalidOperationException("There is already a book type with the given name.");

            // Update the genre's name and isActive properties.
